...
public class GenSign {
	...
	public static String genSign(JSONObject reqObj, String key) {
		...
		System.out.println("sign签名原串 -----:" + sign_src);
	...
	public static String genPartnerSign(JSONObject reqObj, String key) {
		String sign_type = reqObj.getString("partner_sign_type");
		// 生成待签名串
		String sign_src = genSignData(reqObj);
		System.out.println("partner_sign签名原串 -----:" + sign_src);
	...
	public static String getSignRSA(String signSrc, String key) {
		return RSAUtil.sign(key, signSrc);
	}
	...
	public static String genSignData(JSONObject jsonObject) {
		...
		for (int i = 0; i < keys.size(); i++) {
			...
			// 空串不参与签名
			if (null == value || value.length() == 0 || value.equals("")) {
				continue;
			}
