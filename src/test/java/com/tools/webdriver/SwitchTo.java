...
public class SwitchTo {
	public static void frame(WebDriver driver, int id) {
		...
		for (int i = 0; i < Integer.parseInt(Environment
			...
			try {
				...
				Logger.getLogger(SwitchTo.class.getName()).log(
						Level.INFO,
						"driver.switchTo().frame(" + id
								+ ") not found:. time :" + i);
			}
			try {
				...
				Logger.getLogger(SwitchTo.class.getName()).log(Level.SEVERE,
						null, ex);
			}
		}
		throw new org.openqa.selenium.NoSuchFrameException(
				"driver.switchTo().frame(" + id + ")");
	}
	...
	public static void window(WebDriver driver, String title) {
		for (int i = 0; i < Integer.parseInt(Environment
			...
			Logger.getLogger(SwitchTo.class.getName()).log(
					...
					"SwitchTo.window(" + title
							+ "). No windows have the title :" + title
							+ "- failed -time:" + i);
			try {
				Thread.sleep(1000);
			} catch (InterruptedException ex) {
		...
		throw new org.openqa.selenium.NoSuchFrameException("SwitchTo.window("
				+ title + "). No windows have the title :" + title);
	}
	...
	public static void window(WebDriver driver, By locater) {
		...
		for (int i = 0; i < Integer.parseInt(Environment
			...
			Logger.getLogger(SwitchTo.class.getName()).log(
					Level.INFO,
					"could not find the window that have the element :"
							+ locater.toString() + " -time:" + i);
			try {
				Thread.sleep(1000);
			} catch (InterruptedException ex) {
				Logger.getLogger(SwitchTo.class.getName()).log(Level.SEVERE,
	...
	public static void windowWithContent(WebDriver driver, String bodycontent) {
		for (int i = 0; i < Integer.parseInt(Environment
			...
			for (String handle : driver.getWindowHandles()) {
				driver.switchTo().window(handle);
				if (StringUtils.contains(driver.findElement(By.tagName("body"))
						.getText(), bodycontent)) {
					return;
				}
			}
			...
			Logger.getLogger(SwitchTo.class.getName()).log(
					Level.INFO,
					"could not find the window thatbody have string:"
							+ bodycontent + " -time:" + i);
			try {
				Thread.sleep(1000);
			} catch (InterruptedException ex) {
		...
		throw new org.openqa.selenium.NoSuchFrameException(
				"could not find the window thatbody have string:" + bodycontent);
	}
	...
	public static void windowWithUrl(WebDriver driver, String url) {
		for (int i = 0; i < Integer.parseInt(Environment
			...
			Logger.getLogger(SwitchTo.class.getName()).log(
					Level.INFO,
					"could not find the window that url is:" + url + " -time:"
							+ i);
			try {
				Thread.sleep(1000);
			} catch (InterruptedException ex) {
		...
		throw new org.openqa.selenium.NoSuchFrameException(
				"could not find the window that url is:" + url);
	}
}
