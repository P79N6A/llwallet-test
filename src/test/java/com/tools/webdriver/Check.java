...
public class Check {
	...
	public static void Title(WebDriver Driver, String Title) {
		...
		int index = 0;
		while (tag == 0
				&& index < Integer.parseInt(Environment
			...
			if (Title.equals(Driver.getTitle())) {
				...
				try {
					...
					Logger.getLogger(Check.class.getName()).log(Level.SEVERE,
							null, ex);
				}
			}
			index++;
		}
		throw new IllegalStateException("The page title is not " + Title
				+ "but :" + Driver.getTitle());
	}
	...
	public static boolean checkElementExist(WebDriver dr, By locater) {
		return checkElementExist(dr, locater,
				Integer.parseInt(Environment.get("Selenium.waittime")));
	}
	...
	public static boolean checkElementExist(WebDriver dr, By locater, int time) {
		...
		for (int i = 0; i < time; i++) {
			...
			try {
				...
				Logger.getLogger(Check.class.getName()).log(Level.SEVERE, null,
						ex);
			}
		}
		dr.manage()
