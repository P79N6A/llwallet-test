...
public class RefundTest {
	...
	@Test(description = "退款测试", timeOut = 60000, dataProvider = "provider")
	public void test(LinkedHashMap<String, LinkedHashMap<String, String>> dt, ExcelProvider2 excel) throws Exception {
		...
		String no_orderConsumer = JSONObject.parseObject(rsp1).getString("outTradeNo");

		// 退款
		JSONObject refundJson = new JSONObject();
		refundJson.put("merchantNo", dt.get("createQr").get("refund.merchantNo"));
		refundJson.put("version", dt.get("createQr").get("refund.version"));
		refundJson.put("requestDateTime", MyConfig.getDtOrder(dt.get("createQr").get("refund.requestDateTime")));
		refundJson.put("outTradeNo", MyConfig.getNoOrder(dt.get("createQr").get("refund.outTradeNo")));
		refundJson.put("outTradeDate", MyConfig.getDtOrder(dt.get("createQr").get("refund.outTradeDate")));
		refundJson.put("signData", dt.get("createQr").get("refund.signData"));
		refundJson.put("signType", dt.get("createQr").get("refund.signType"));
		refundJson.put("tradeAmount", dt.get("createQr").get("refund.tradeAmount"));
		refundJson.put("originalOutTradeNo", no_orderConsumer);// 原消费流水外部单号
	    //refundJson.put("originalOutTradeNo", "ConsumerBill20180305160811");//传入已退款成功的消费流水外部单号，进行重复退款
	    //refundJson.put("originalOutTradeNo", "ConsumerBill20180305160811");//传入未退款成功的消费流水外部单号，进行重复退款
	    //refundJson.put("originalOutTradeNo", "ConsumerBill20180123185429");//进行并发退款
	    //refundJson.put("originalOutTradeNo", "ConsumerBill20180305172548");//原消费消费流水外部单号
		refundJson.put("elecCardNo", dt.get("createQr").get("refund.elecCardNo"));
		refundJson.put("elecBankCode", dt.get("createQr").get("refund.elecBankCode"));
		refundJson.put("tradeSubject", "连连测试kkghdfdsgj");
		refundJson.put("returnParam ", "5416");
		refundJson.put("extParam", "afdghfgjhfs");

		String refundUrl = Property.get("tpay.refund.url");
		String rsp2 = HttpRequest.httpPostWithJSON(refundUrl, refundJson.toString());

		Reporter.log("退款测试： " + dt.get("createQr").get("comment"), true);
		System.out.println("===============================================");

		// 请求&响应写入文件
		SampleFileUtils.appendLine("D://TA//log//refund.txt", MyDate.getStringDate());
		SampleFileUtils.appendLine("D://TA//log//refund.txt", reqJson);
		SampleFileUtils.appendLine("D://TA//log//refund.txt", rsp);
		SampleFileUtils.appendLine("D://TA//log//refund.txt", json.toString());
		SampleFileUtils.appendLine("D://TA//log//refund.txt", rsp1);
		SampleFileUtils.appendLine("D://TA//log//refund.txt", refundJson.toString());
		SampleFileUtils.appendLine("D://TA//log//refund.txt", rsp2);
		SampleFileUtils.appendLine("D://TA//log//refund.txt", "===============================================");
