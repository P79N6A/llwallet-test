...
public class Balance2OtherBankCardTest {
	...
	public void test(LinkedHashMap<String, LinkedHashMap<String, String>> dt, ExcelProvider2 excel) throws Exception {
		...
		if (dt.get("balance2OtherBankCard").get("expect_return").contains("支付密码错误")) {
			DbUtils dbUtils = new DbUtils();
			String oid_userno = dbUtils
					.queryOidAcctNo(MyConfig.getPartner(dt.get("balance2OtherBankCard").get("oid_partner")),
							MyConfig.getUser(dt.get("balance2OtherBankCard").get("user_id")))
					.getOid_acctno().trim();
		...
		String balance2OtherBankCardUrl = Property.get("llwallet.balance2OtherBankCard.url");
		// 用户余额到他人银行卡请求提交
	...
	@SuppressWarnings("unchecked")
	@Test(description = "余额到他人银行卡接口测试", timeOut = 60000, dataProvider = "provider1")
	public void baseline(LinkedHashMap<String, LinkedHashMap<String, String>> dt, ExcelProvider2 excel)
		...
		SampleFileUtils.appendLine("D://TA//log//balance2OtherBankCard.txt", MyDate.getStringDate());
		SampleFileUtils.appendLine("D://TA//log//balance2OtherBankCard.txt", reqJson);
		SampleFileUtils.appendLine("D://TA//log//balance2OtherBankCard.txt", rsp);
		SampleFileUtils.appendLine("D://TA//log//balance2OtherBankCard.txt",
				"===============================================");
		// 支付请求返回检查
		...
		Reporter.log("用户余额到他人银行卡接口测试： " + dt.get("baseline").get("comment"), true);
		System.out.println("===============================================");
