...
public class QueryOnlineBankRechargeLimitedTest extends AbstractTestNGSpringContextTests {
	...
	@Test(description = "查询网银充值限额是否超限接口测试", timeOut = 60000, dataProvider = "queryOnlineBankRechargeLimited")
	public void queryOnlineBankRechargeLimited(Map<String, String> datadriven) throws Exception {
		...
		String rsp = JSON.toJSONString(iWalletRechargeBusiService.queryOnlineBankRechargeLimited(walletRechargeBean));
		System.out.println("查询网银充值限额是否超限返回：： " + rsp);
		Reporter.log("查询网银充值限额是否超限返回：" + rsp);
		// 查询网银充值限额是否超限请求返回检查
		assert rsp.contains(datadriven.get("result_code"));

		Reporter.log("查询网银充值限额是否超限接口测试： " + datadriven.get("comment"), true);
		System.out.println("===============================================");
	}

	@DataProvider(name = "queryOnlineBankRechargeLimited")
	public Iterator<Object[]> data4queryOnlineBankRechargeLimited() throws IOException {
		return new ExcelProvider(this, "queryOnlineBankRechargeLimited");
	}
