...
public class UpdateRechargeLimitedTest extends AbstractTestNGSpringContextTests {
	...
	@Test(description = "更新银行卡充值限额接口测试", timeOut = 60000, dataProvider = "updateRechargeLimited")
	public void updateRechargeLimited(Map<String, String> datadriven) throws Exception {
		...
		String rsp = JSON.toJSONString(iWalletRechargeBusiService.updateRechargeLimited(walletRechargeBean));
		System.out.println("更新银行卡充值限额返回：： " + rsp);
		Reporter.log("更新银行卡充值限额返回：" + rsp);
		// 更新银行卡充值限额请求返回检查
		assert rsp.contains(datadriven.get("result_code"));

		Reporter.log("更新银行卡充值限额接口测试： " + datadriven.get("comment"), true);
		System.out.println("===============================================");
	}

	@DataProvider(name = "updateRechargeLimited")
	public Iterator<Object[]> data4updateRechargeLimited() throws IOException {
		return new ExcelProvider(this, "updateRechargeLimited");
	}
