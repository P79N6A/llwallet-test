...
public class TrustAuthTest extends AbstractTestNGSpringContextTests {
	...
	@Test(description = "实名认证接口测试", timeOut = 60000, dataProvider = "trustAuth")
	public void trustAuth(Map<String, String> datadriven) throws Exception {
		...
		String rsp = JSON.toJSONString(llwalletCoreService.trust_auth(trustAuth));
		System.out.println("实名认证返回：" + rsp);
		Reporter.log("实名认证返回：" + rsp);
		// 实名认证请求返回检查
		assert rsp.contains(datadriven.get("result_code"));

		Reporter.log("实名认证接口测试： " + datadriven.get("comment"), true);
		System.out.println("===============================================");
	}

	@DataProvider(name = "trustAuth")
	public Iterator<Object[]> data4trustAuth() throws IOException {
		return new ExcelProvider(this, "trustAuth",1);
	}
