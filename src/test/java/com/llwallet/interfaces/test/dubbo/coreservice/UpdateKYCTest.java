...
public class UpdateKYCTest extends AbstractTestNGSpringContextTests {
	...
	@Test(description = "异步更新KYC等级接口测试", timeOut = 60000, dataProvider = "updateKYC")
	public void updateKYC(Map<String, String> datadriven) throws Exception {
		...
		String rsp = JSON.toJSONString(llwalletCoreService.updateKYC(trustAuth));
		System.out.println("异步更新KYC等级返回：" + rsp);
		Reporter.log("异步更新KYC等级返回：" + rsp);
		// 异步更新KYC等级请求返回检查
		assert rsp.contains(datadriven.get("result_code"));

		Reporter.log("异步更新KYC等级接口测试： " + datadriven.get("comment"), true);
		System.out.println("===============================================");
	}

	@DataProvider(name = "updateKYC")
	public Iterator<Object[]> data4updateKYC() throws IOException {
		return new ExcelProvider(this, "updateKYC");
	}
