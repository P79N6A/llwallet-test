...
public class QueryRechargeLimitedTest extends AbstractTestNGSpringContextTests {
	...
	@Test(description = "查询银行卡充值限额是否超限接口测试", timeOut = 60000, dataProvider = "queryRechargeLimited")
	public void queryRechargeLimited(Map<String, String> datadriven) throws Exception {
		...
		String rsp = JSON.toJSONString(iWalletRechargeBusiService.queryRechargeLimited(walletRechargeBean));
		System.out.println("查询银行卡充值限额是否超限返回：： " + rsp);
		Reporter.log("查询银行卡充值限额是否超限返回：" + rsp);
		// 查询银行卡充值限额是否超限请求返回检查
		assert rsp.contains(datadriven.get("result_code"));

		Reporter.log("查询银行卡充值限额是否超限接口测试： " + datadriven.get("comment"), true);
		System.out.println("===============================================");
	}

	@DataProvider(name = "queryRechargeLimited")
	public Iterator<Object[]> data4queryRechargeLimited() throws IOException {
		return new ExcelProvider(this, "queryRechargeLimited");
	}
